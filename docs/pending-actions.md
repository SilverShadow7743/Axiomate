...
| F5 | ~~No staging slot, and none possible~~ | **Mostly done, 7 Sep.** Slot + P0v3 (done). Entra identity `axiomate-tms-deploy` created with the federated credential and `Website Contributor` (done) — PostgreSQL firewall-rule rights still pending (Nishant granting directly, in progress). Basic publishing credentials disabled (done). What's left: the PostgreSQL role, then §2.2's GitHub `production` environment secrets (`AZURE_CLIENT_ID` = `74f381dd-fd4e-4fb7-b9b7-421c977566ef`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `DATABASE_URL`), then removing the `deploy` job's `workflow_dispatch`-only gate. Releases still ship by hand until all three land |
