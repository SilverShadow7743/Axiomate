...
| ~~F5~~ | ~~No staging slot, and none possible~~ | **Done, 7 Sep.** Slot + P0v3, Entra identity `axiomate-tms-deploy` with the federated credential, `Website Contributor` on the App Service, and `Contributor` on `axiomate-tms-db` (verified via direct ARM role-assignment read after a first grant landed on the wrong principal) are all in place. Basic publishing credentials disabled. GitHub `production` environment secrets set (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `DATABASE_URL`). The `deploy` job's `workflow_dispatch`-only gate is removed — a push to main/master now deploys through the slot automatically |
