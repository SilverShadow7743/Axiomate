...
| A7 | **Accept the guest invitation** *(added 23 Aug, deferred same day; actually invited 31 Aug — this row was stale; checked live via Graph API 2026-09-06 and resent)* | The guest object exists (`nishant.ax@gmail.com`, `userType: Guest`, tenant id `f70e0543-3116-41d4-b0fa-d66e60a19da6`) and is `externalUserState: PendingAcceptance` — the invite was sent, nobody has clicked Accept. Resent 2026-09-06 via `POST /v1.0/invitations` (reused the same guest object, did not duplicate it). What's left is entirely on the gmail side: open the invitation email in `nishant.ax@gmail.com`, accept it, then sign in from a separate browser profile. The app side is DONE and waiting: PERSON_85 sits in the directory (Client User, that exact email, scoped to OAPIL), the scoped boundary is deployed, and GA1 proves it. This one action is also where the `#EXT#` email-claim question gets its real answer |
