...
| A7 | **The invitation email is not reaching Gmail — a delivery problem, not a "go accept it" task** *(added 23 Aug; actually invited 31 Aug and resent 2026-09-06, both confirmed via Graph API; Gmail searched directly 2026-09-07 via the connected Gmail account — inbox, spam, trash, all folders, no date limit, multiple query variants — zero matching messages, from either send)* | The guest object is real (`nishant.ax@gmail.com`, `userType: Guest`, tenant id `f70e0543-3116-41d4-b0fa-d66e60a19da6`, `externalUserState: PendingAcceptance`) and Entra confirms two invitation sends. Neither email exists anywhere in the destination Gmail account — not even in spam, which points at delivery failing before it reaches Google's mail servers (sender reputation, SPF/DKIM/DMARC, or the tenant's outbound mail flow), not at Gmail filtering it. Accepting an email that was never delivered isn't the blocker; **finding out why Entra's B2B invitation mail isn't leaving the building is.** Candidates worth checking: the tenant's mail-flow/connector settings, whether `invites@microsoft.com`/`azure-noreply@microsoft.com` sending is throttled or blocked for this tenant, or a bounce/NDR that would show up in the sender side (Entra doesn't surface delivery failures for invitations the way a mailbox would). The app side is still DONE and waiting: PERSON_85, the scoped boundary, GA1 |
