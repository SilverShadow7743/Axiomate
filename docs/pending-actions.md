...
| I12 | **"Why this works this way" — Axiocloud's own design docs, surfaced in the app instead of a Hive-University-style course library** | Brainstormed 8 Sep from a comparison against Hive University (a separate LMS site — video courses, webinars, a "Center of Excellence"). A direct clone doesn't fit — that's content production, a different kind of work — but most of the underlying job (get oriented, understand *why*) is already served differently and more honestly here: computed checklists (I6) instead of a course, inline `cfg-note` prose instead of a separate page. Design drafted: `docs/plans/2026-09-08-why-this-works-design.md` — a curated (not crawled) small set of existing `docs/plans/*.md` files, copied to `public/help/` at build time (mirroring the existing `data/` copy step) since `docs/` is not currently shipped in the deploy package, with one small "Why this works this way" link per curated screen. **Not built.** Two open questions for Nishant in the design: markdown-rendering dependency vs. plain text, and which docs make the first curated list |
