...
| F5 | ~~No staging slot, and none possible~~ | **Half done, 7 Sep.** `axiomate-tms-plan` moved to P0v3 and a `staging` slot exists (`axiomate-tms-staging.azurewebsites.net`) — the physical precondition is met. The deploy job still can't run: it's gated `if: github.event_name == 'workflow_dispatch'` and, per its own comment, needs two more things nobody has set up yet — an Entra app registration + federated credential, and the GitHub `production` environment's secrets (DATABASE_URL, OIDC client). Releases still ship by hand until those exist |
