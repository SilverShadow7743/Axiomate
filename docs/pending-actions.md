...
| I16 | **See an email's actual content, not just the truncated preview** | Scoped 8 Sep, not built. Design: `docs/plans/2026-09-08-email-body-view-design.md`. Fetch-on-open (`GET /api/mail/message/[id]`), same RAM-only/never-stored posture as the rest of `in-mail`, no new Entra consent. The one real decision: render HTML bodies inside a sandboxed `<iframe srcdoc>` (no `allow-scripts`) rather than `dangerouslySetInnerHTML` — untrusted sender content is a genuine XSS surface otherwise. Inline images (`cid:`) and attachment download stay out, named as follow-ons |
