...
| I11 | **Configuration → Automation has no "Add rule" control at all** | Found live, 8 Sep, while adding `AUTO_SUBWORK_CLOSED` (I10) to production the intended way — `ConfigWorkspace.tsx`'s `Automation` component only maps over and edits `state.model.automationRules`; there is no button anywhere, in the UI or the code, to create a new rule. Only the shipped defaults exist and are editable in place. Worked around for I10 with a direct config write (Nishant's explicit approval); the real UI gap is still open. **Not built** — Nishant chose to defer this rather than build it first |
