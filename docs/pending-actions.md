...
| I15 | **Leave report, month on month** | Scoped 8 Sep, not built. Design: `docs/plans/2026-09-08-leave-report-design.md`. A new report (`lib/reports/leave.ts`) — the finance report explicitly excludes leave, so this is a fifth report, not an extension. Grid shape (person × month, working days taken, holiday-aware), `Approved`-only via the existing `commitmentCounts` rule, reason text excluded from the export entirely (stricter than the Capacity screen's `mayReadReasons` gate — named as a decision to confirm). Three open questions for Nishant: which months, whether reason exclusion is right, whether project-level attribution is needed (the model doesn't carry it today) |
