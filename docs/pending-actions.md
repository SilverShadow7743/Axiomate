...
| I14 | **An issue's own due date should carry a visible caveat when its owner is on approved leave — never a silent shift** | Follow-up from I13's third open question, approved to scope (not build) 8 Sep. Design: `docs/plans/2026-09-08-leave-aware-due-date-design.md`. The answer is already sourced from this codebase's own stated doctrine, not invented here: `lib/sla.ts`'s own header — *"A date already here is a commitment somebody made. Never overwritten."* — and `lib/scheduling.ts`'s leave posture — approved leave "blocks nothing... a named conflict," never a silent reschedule. So this is a computed caveat surfaced next to the due date (current `owner` only, not every future assignee — the one concrete, known fact, consistent with the "no speculative derived value" doctrine already enforced in `lib/goals.ts`/`lib/portfolio.ts`/`lib/skills.ts`'s `candidatesFor`), never a rewrite of `plannedEndDate`. Attachment point named: `DetailPanel.tsx`'s Overview tab, beside "Planned end". **Not built.** Narrow open question: which screen(s) carry the caveat beyond that one, and whether `watch.ts` treats it as caveat-only (display) or lets it affect `dueSoon`'s own timing |
